...
// ─── API calls (vraie API Spotify via Vercel Serverless Functions) ──────────────
async function fetchArtist(query) {
  const endpoint = `/api/spotify-artist?q=${encodeURIComponent(query)}`;
...
function getLevel(s) {
  ...
  if (s < 75) return { label: "En Traction 🔥", color: "#f59e0b", bg: "rgba(245,158,11,0.12)", tip: "L'algorithme te recommande activement. Maintiens le momentum avec des campagnes et des sorties régulières !" };
  return { label: "Artiste Populaire ⭐", color: "#1DB954", bg: "rgba(29,185,84,0.12)", tip: "Tu es dans le top tier Spotify ! Excellent positionnement algorithmique et forte recommandation organique." };
}
...
function Gauge({ score }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
        ...
        <svg width={172} height={172} style={{ transform: "rotate(-90deg)" }}>
          <defs>
            <linearGradient id="arcGrad" x1="0%" y1="0%" x2="100%" y2="0%">
              <stop offset="0%" stopColor={arc} stopOpacity="0.22" />
          ...
          <circle cx={86} cy={86} r={R} fill="none" stroke="url(#arcGrad)" strokeWidth={sw}
            strokeDasharray={C} strokeDashoffset={C - prog} strokeLinecap="round"
            filter="url(#arcGlow)" style={{ transition: "stroke-dashoffset 0.05s linear" }} />
        </svg>
      ...
      <div style={{ padding: "5px 16px", borderRadius: 20, background: lv.bg, border: `1px solid ${lv.color}38`, fontSize: 11, fontWeight: 700, color: lv.color, letterSpacing: 0.3 }}>
        {lv.label}
      </div>
    </div>
  );
...
function Track({ track, index, delay }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 11, padding: "9px 13px", borderRadius: 10, background: "rgba(255,255,255,0.022)", border: "1px solid rgba(255,255,255,0.05)", opacity: vis ? 1 : 0, transform: vis ? "translateX(0)" : "translateX(-18px)", transition: "all 0.38s ease" }}>
      {/* Album art */}
      {track.albumImage ? (
        <img src={track.albumImage} alt={track.album} style={{ width: 34, height: 34, borderRadius: 6, objectFit: "cover", flexShrink: 0 }} />
      ) : (
      ...
      {/* Preview player si disponible */}
      {track.previewUrl && (
        <button onClick={() => { const a = new Audio(track.previewUrl); a.play(); }}
          style={{ background: "rgba(29,185,84,0.15)", border: "1px solid rgba(29,185,84,0.3)", color: "#1DB954", width: 26, height: 26, borderRadius: "50%", cursor: "pointer", fontSize: 10, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          ▶
        </button>
      )}
    </div>
...
function Card({ o }) {
  ...
  return (
    <div onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
      ...
      <div style={{ padding: "8px 0", borderRadius: 8, textAlign: "center", background: o.hot ? "#1DB954" : "rgba(255,255,255,0.06)", color: o.hot ? "#000" : "rgba(255,255,255,0.55)", fontSize: 10, fontWeight: 700, letterSpacing: 0.3 }}>
        Booster →
      </div>
    </div>
  );
...
export default function App() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#07090f", color: "white", fontFamily: "'DM Sans','Helvetica Neue',sans-serif", position: "relative", overflow: "hidden" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Bebas+Neue&family=DM+Sans:opsz,wght@9..40,300;9..40,400;9..40,500;9..40,600;9..40,700;9..40,800;9..40,900&display=swap');
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 680, margin: "0 auto", padding: "0 18px 70px" }}>
        ...
        <div style={{ textAlign: "center", padding: "42px 0 26px", animation: "fadeUp 0.5s ease" }}>
          <div style={{ fontSize: 9, letterSpacing: 4.5, color: "#1DB954", textTransform: "uppercase", fontFamily: "monospace", marginBottom: 10, animation: "pulse 2.2s ease infinite" }}>
            ● GRATUIT · INSTANTANÉ · OFFICIEL
          </div>
          ...
          <div style={{ animation: "fadeUp 0.5s ease 0.1s both" }}>
            <div style={{ display: "flex", gap: 6, background: "rgba(255,255,255,0.034)", border: `1px solid ${focused ? "rgba(29,185,84,0.44)" : "rgba(255,255,255,0.088)"}`, borderRadius: 12, padding: "5px 5px 5px 14px", transition: "all 0.2s", boxShadow: focused ? "0 0 0 3px rgba(29,185,84,0.07)" : "none" }}>
              ...
              <input ref={ref} value={q} onChange={e => setQ(e.target.value)} onKeyDown={e => e.key === "Enter" && go()} onFocus={() => setFocused(true)} onBlur={() => setFocused(false)}
                placeholder="Nom d'artiste ou lien Spotify..."
                style={{ flex: 1, background: "transparent", border: "none", color: "white", fontSize: 13, fontFamily: "'DM Sans',sans-serif", padding: "9px 0" }} />
              <button onClick={go} style={{ background: q.trim() ? "#1DB954" : "rgba(255,255,255,0.055)", color: q.trim() ? "#000" : "rgba(255,255,255,0.22)", border: "none", borderRadius: 8, padding: "10px 17px", fontSize: 11, fontWeight: 800, cursor: q.trim() ? "pointer" : "default", letterSpacing: 0.3, transition: "all 0.2s", whiteSpace: "nowrap" }}>
                Analyser →
              </button>
            </div>
            <div style={{ display: "flex", gap: 6, marginTop: 9, flexWrap: "wrap", justifyContent: "center" }}>
              {["BLVG 1977", "Drake", "Aya Nakamura", "Burna Boy"].map(n => (
        ...
        {phase === "result" && artist && (
          <div style={{ animation: "fadeUp 0.45s ease" }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.023)", border: "1px solid rgba(255,255,255,0.072)", borderRadius: 17, padding: "22px 18px", marginBottom: 12 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 10 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 13 }}>
                  {/* Artist photo (réelle depuis Spotify) */}
                  {artist.image ? (
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 10, color: "rgba(255,255,255,0.28)", fontFamily: "monospace" }}>
                      {fmt(artist.followers)} followers · {artist.genres.slice(0, 2).join(", ") || "—"}
                    </div>
                    {artist.externalUrl && (
                      <a href={artist.externalUrl} target="_blank" rel="noreferrer" style={{ fontSize: 9, color: "#1DB954", textDecoration: "none", fontFamily: "monospace", letterSpacing: 0.5 }}>
                        ↗ Voir sur Spotify
                      </a>
                    )}
                  </div>
                </div>
                <button onClick={reset} style={{ background: "rgba(255,255,255,0.038)", border: "1px solid rgba(255,255,255,0.085)", color: "rgba(255,255,255,0.32)", fontSize: 9, padding: "6px 12px", borderRadius: 7, cursor: "pointer", fontFamily: "monospace", letterSpacing: 0.8 }}>
                  ← NOUVELLE RECHERCHE
                </button>
              </div>
              ...
              <div style={{ display: "flex", gap: 20, alignItems: "center", flexWrap: "wrap", justifyContent: "center" }}>
                ...
                <div style={{ flex: 1, minWidth: 188 }}>
                  ...
                  <div style={{ padding: "11px 13px", borderRadius: 10, background: lv.bg, border: `1px solid ${lv.color}26`, fontSize: 11, color: "rgba(255,255,255,0.62)", lineHeight: 1.65, marginBottom: 10 }}>
                    {lv.tip}
                  </div>
            ...
            <div style={{ background: "rgba(255,255,255,0.016)", border: "1px solid rgba(255,255,255,0.052)", borderRadius: 17, padding: "17px", marginBottom: 12 }}>
              <div style={{ fontSize: 8, color: "rgba(255,255,255,0.28)", fontFamily: "monospace", letterSpacing: 2.5, marginBottom: 12, textTransform: "uppercase" }}>
                ▸ Top Titres par Popularité
              </div>
